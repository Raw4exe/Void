...
local old_Zen = CoreGui:FindFirstChild('Zen')

if old_Zen then
    Debris:AddItem(old_Zen, 0)
end

if not isfolder("Zen") then
    makefolder("Zen")
end
...
local Config = setmetatable({
    save = function(self: any, file_name: any, config: any)
        local success_save, result = pcall(function()
            ...
            writefile('Zen/'..file_name..'.json', flags)
        end)
    ...
    load = function(self: any, file_name: any, config: any)
        local success_load, result = pcall(function()
            if not isfile('Zen/'..file_name..'.json') then
                self:save(file_name, config)
        
                return
            end
        
            local flags = readfile('Zen/'..file_name..'.json')
...
function Library:create_ui()
    ...
    local Zen = Instance.new('ScreenGui')
    Zen.ResetOnSpawn = false
    Zen.Name = 'Zen'
    Zen.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    Zen.Parent = CoreGui
    ...
    function self:load()
        ...
        for _, object in Zen:GetDescendants() do
    ...
    function self:create_tab(title: string, icon: string)
        ...
        function TabManager:create_module(settings: any)
            ...
            if not settings.rich then
                ...
                ModuleName.Text = settings.richtext or "<font color='rgb(255,0,0)'>Zen</font> user"
            end;
            ...
            function ModuleManager:create_paragraph(settings: any)
                ...
                if not settings.rich then
                    ...
                    Body.RichText = true
                    Body.Text = settings.richtext or "<font color='rgb(255,0,0)'>Zen</font> user"
                end
            ...
            function ModuleManager:create_text(settings: any)
                ...
                function TextManager:Set(new_settings)
                    if not new_settings.rich then
                        ...
                        Body.RichText = true
                        Body.Text = new_settings.richtext or "<font color='rgb(255,0,0)'>Zen</font> user"
                    end
                end;
