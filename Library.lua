...
function convertTableToString(inputTable)
    ...
    for _, value in pairs(inputTable) do
        ...
        elseif typeof(value) == "table" and value.Name then
...
function validateValue(value, expectedType)
    ...
    if expectedType == "table" and typeof(value) == "string" then
        return convertStringToTable(value)
    end
    
    if expectedType == "string" and typeof(value) == "table" then
        return convertTableToString(value)
    end
...
local Config = setmetatable({
    save = function(self: any, file_name: any, config: any)
        if not config or typeof(config) ~= "table" then
        ...
        local success_save, result = pcall(function()
            ...
            if flags and typeof(flags) == "string" then
                writefile('March/'..tostring(file_name)..'.json', flags)
                return true
            end
            return false
        end)
    ...
    load = function(self: any, file_name: any, config: any)
        local success_load, result = pcall(function()
            local fileName = 'March/'..tostring(file_name)..'.json'
            
            if not isfile(fileName) then
                self:save(file_name, config or {_flags = {}, _keybinds = {}, _library = {}})
                return config or {_flags = {}, _keybinds = {}, _library = {}}
            end
            ...
            if not decoded._flags then decoded._flags = {} end
            if not decoded._keybinds then decoded._keybinds = {} end
            if not decoded._library then decoded._library = {} end
        ...
        if not result or typeof(result) ~= "table" then
            result = {
                _flags = {},
                _keybinds = {},
                _library = {}
        ...
        function TabManager:create_module(settings: any)
            ...
            function ModuleManager:create_checkbox(settings: any)
                ...
                function CheckboxManager:change_state(state: boolean)
                    ...
                    if settings.callback and typeof(settings.callback) == "function" then
                        safeCallback(settings.callback, self._state)
                    end
                end
            ...
            function ModuleManager:create_slider(settings: any)
                ...
                function SliderManager:set_percentage(percentage: number)
                    percentage = tonumber(percentage) or settings.minimum_value
                    ...
                    if settings.callback and typeof(settings.callback) == "function" then
                        safeCallback(settings.callback, number_threshold)
                    end
                end
            ...
            function ModuleManager:create_dropdown(settings: any)
                ...
                if not settings.maximum_options then
                    settings.maximum_options = #settings.options
                end
                ...
                function DropdownManager:update(option: string)
                    if settings.multi_dropdown then
                        ...
                        if typeof(Library._config._flags[settings.flag]) ~= "table" then
                            Library._config._flags[settings.flag] = {}
                        end

                        local CurrentTargetValue = nil
                        
                        if typeof(Library._config._flags[settings.flag]) == "table" and #Library._config._flags[settings.flag] > 0 then
                            CurrentTargetValue = convertTableToString(Library._config._flags[settings.flag])
                        end

                        local selected = {}

                        if CurrentTargetValue and typeof(CurrentTargetValue) == "string" then
                            selected = convertStringToTable(CurrentTargetValue)
                        elseif typeof(CurrentOption.Text) == "string" and CurrentOption.Text ~= "" then
                            selected = convertStringToTable(CurrentOption.Text)
                        end
                        
                        local cleanSelected = {}
                        for _, v in pairs(selected) do
                            if typeof(v) == "string" and v ~= "Label" and v ~= "" then
                                table.insert(cleanSelected, v)
                            end
                        end
                        selected = cleanSelected
                
                        local optionName = ""
                        if typeof(option) == "table" and option.Name then
                        ...
                        local validSelected = {}
                        for _, v in pairs(selected) do
                            if table.find(OptionsChild, v) then
                                table.insert(validSelected, v)
                            end
                        end

                        CurrentOption.Text = table.concat(validSelected, ", ")
                
                        Library._config._flags[settings.flag] = validSelected
                    else
                        local optionValue = ""
                        if typeof(option) == "table" and option.Name then
                        ...
                        CurrentOption.Text = optionValue
                        ...
                        Library._config._flags[settings.flag] = optionValue
                    end
                    ...
                    if settings.callback and typeof(settings.callback) == "function" then
                        local callbackValue
                        if settings.multi_dropdown then
                            callbackValue = Library._config._flags[settings.flag]
                        else
                            callbackValue = Library._config._flags[settings.flag]
                        end
                        
                        safeCallback(settings.callback, callbackValue)
                    end
                end
                ...
                if typeof(settings.options) == "table" and #settings.options > 0 then
